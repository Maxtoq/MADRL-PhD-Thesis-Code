...
class Switch(gym.Env):
    ...
    def __init__(self, full_observable: bool = False, step_cost: float = 0, n_agents: int = 4, max_steps: int = 50,
        ...
        # agent pos (2)
        self._obs_high = np.ones(2 + (1 if self._add_clock else 0), dtype=np.float32)
        self._obs_low = np.zeros(2 + (1 if self._add_clock else 0), dtype=np.float32)
        ...
        self._shared_obs_high = np.ones(2 * self.n_agents, dtype=np.float32)
        self._shared_obs_low = np.zeros(2 * self.n_agents, dtype=np.float32)
