...
class OneHotEncoder:
    ...
    def decode_batch(self, enc_sent_batch):
        ...
        Inputs:
            :param enc_sent_batch (list): List of encoded sentences.
        Outputs:
            :param decoded_batch (list): List of sentences.
        """
        decoded_batch = []
        for enc_sentence in enc_sent_batch:
            sentence = []
...
class GRUDecoder(nn.Module):
    ...
    def __init__(self, context_dim, embed_dim, token_dim, max_len, 
                 n_layers=1, embed_layer=None, use_gumbel=False, device="cpu"):
        """
        Inputs:
            ...
            :param word_encoder (OneHotEncoder): Word encoder, associating 
                tokens with one-hot encodings
            :param n_layers (int): number of layers in the GRU (default: 1)
            :param embed_layer: (torch.nn.Embedding) already defined Embedding 
                layer, default=None.
            :param use_gumbel: (bool) whether to make the decoder fully 
                differentiable, with the use of gumbel softmax when generating 
        ...
        self.out = nn.Sequential(
            init_(nn.Linear(self.hidden_dim, token_dim)),
            nn.LogSoftmax(dim=2)
        )
        # self.out = init_(nn.Linear(self.hidden_dim, token_dim))
        if self.use_gumbel:
            self.tau_gen = init_(nn.Linear(self.hidden_dim, 1))
            self.tau_zero = 0.5
    ...
    def forward(self, context_batch, target_encs=None):
        ...
        Outputs:
            :param decoder_outputs (torch.Tensor): Batch of tensors containing
                log-probabilities generated by the GRU network.
            :param sentences (torch.Tensor): Sentences generated with greedy 
        ...
        for t_i in range(max_sent_len):
            ...
            if teacher_forcing:
                ...
                if self.use_gumbel: # TODO Fix for differentiable version, does not work because topk() does not pass gradients (don't output indices, only one hots)
                    ...
                    last_tokens = torch.matmul(
                        one_hot, self.embed_layer.weight).unsqueeze(0)
                else:
