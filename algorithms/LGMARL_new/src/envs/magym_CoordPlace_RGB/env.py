...
class Env(gym.Env):
    ...
    Coordinated placement: 
        ...
        same color.
    Observations: 
        Agents observe only their position and the color of the
        landmark they are on (if on a landmark).
    Rewards: 
    ...
    def __init__(self, n_agents=2, step_cost=-1.0, max_steps=50):
        ...
        self.lm_colors = [None for _ in range(8)]
        ...
        self._obs_high = np.ones(2 + 3, dtype=np.float32)
        self._obs_low = np.zeros(2 + 3, dtype=np.float32)
        self.observation_space = MultiAgentObservationSpace(
            [spaces.Box(self._obs_low, self._obs_high) for _ in range(self.n_agents)])

        self._shared_obs_high = np.ones((2 + 3) * self.n_agents, dtype=np.float32)
        self._shared_obs_low = np.zeros((2 + 3) * self.n_agents, dtype=np.float32)
    ...
    def __put_landmarks(self):
        for lp, lc in zip(LM_POSITIONS, self.lm_colors):
            r, c = lp
            self._full_obs[r][c] = ENT_IDS['landmark'] + str(LM_COLORS[lc])
            self._full_obs[r + 1][c] = ENT_IDS['landmark'] + str(LM_COLORS[lc])
            self._full_obs[r][c + 1] = ENT_IDS['landmark'] + str(LM_COLORS[lc])
            self._full_obs[r + 1][c + 1] = ENT_IDS['landmark'] + str(LM_COLORS[lc])
    ...
    def __init_landmarks(self):
        ...
        available_landmarks = list(range(8))

        while len(available_landmarks) > 0:
            color = random.choice(available_colors)
            available_colors.remove(color)
            for _ in range(self.n_agents):
                lm_i = random.choice(available_landmarks)
                available_landmarks.remove(lm_i)

                self.lm_colors[lm_i] = color

                if len(available_landmarks) == 0:
                    break
    ...
    def __get_position_lm(self, pos):
        for l_i, l_p in enumerate(LM_POSITIONS):
