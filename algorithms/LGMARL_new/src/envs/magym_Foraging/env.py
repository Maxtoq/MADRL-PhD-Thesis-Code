...
class Env(gym.Env):
    ...
    def __init__(self, grid_shape=(10, 10), n_agents=4, n_gems=10,
        ...
        self.gem_colors = [2 if no_purple else 3, 2, 2]
                ...
                for col in range(max(0, pos[1] - obs_range), min(pos[1] + obs_range + 1, self._grid_shape[1])):
                    if self._full_obs[row][col] != PRE_IDS["empty"]:
                        ...
                        if PRE_IDS['agent'] in self._full_obs[row][col]: # and self._full_obs[row][col][-1] != str(agent_i + 1):
                            _gem_pos[row - (pos[0] - obs_range), col - (pos[1] - obs_range)] = 1
                        elif PRE_IDS['gem'] in self._full_obs[row][col]:
                            _gem_pos[row - (pos[0] - obs_range), col - (pos[1] - obs_range)] = 1 + int(self._full_obs[row][col][-1]) # get relative position for the gem loc.
            _gem_pos /= 4
