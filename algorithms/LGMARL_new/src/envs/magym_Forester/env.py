...
class ForesterEnv(gym.Env):
    """
    Forester involves a grid world, in which multiple foresters manage a land with growing trees.
    Agents have a 5 × 5 view and select one of five actions ∈ {Left, Right, Up, Down, Stop} at each time step.
    Trees spawn randomly on the map every 5 steps. 
    They start as "Yellow Trees" that can be cut by one Forester alone but gives only +1 reward.
    They grow of one level each 10 time steps, going Yellow to Orange: two Foresters needed, +3 reward, 
