...
class E2S_NovelD(RND, E3B):
    ...
    def init_new_episode(self):
        ...
        if self.ablation in [None, "EEC"]:
            E3B.init_new_episode(self)
    
    def set_train(self, device):
        if self.ablation in [None, "LLEC"]:
            RND.set_train(self, device)
        if self.ablation in [None, "EEC"]:
            E3B.set_train(self, device)

    def set_eval(self, device):
        if self.ablation in [None, "LLEC"]:
            RND.set_eval(self, device)
        if self.ablation in [None, "EEC"]:
            E3B.set_eval(self, device)
    ...
    def get_reward(self, state):
        ...
        ## E3B
        if self.ablation in [None, "EEC"]:
            elliptic_scale = E3B.get_reward(self, state)
        else:
            elliptic_scale = 1.0
    ...
    def train(self, state_batch, act_batch):
        ...
        e3b_loss = 0.0
        if self.ablation in [None, "LLEC"]:
            rnd_loss = RND.train(self, state_batch, act_batch)
        if self.ablation in [None, "EEC"]:
            e3b_loss = E3B.train(self, state_batch, act_batch)
        return rnd_loss + e3b_loss
    
    def get_params(self):
        rnd_params = RND.get_params(self)
        e3b_params = E3B.get_params(self)
        return dict(rnd_params, **e3b_params)

    def load_params(self, params):
        RND.load_params(self, params)
        E3B.load_params(self, params)
