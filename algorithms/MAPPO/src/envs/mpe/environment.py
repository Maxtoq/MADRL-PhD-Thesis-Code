...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, scenario, discrete_action=False, shared_viewer=True):
        ...
        self.shared_observation_space = [spaces.Box(
            low=-np.inf, high=+np.inf, shape=(share_obs_dim,), dtype=np.float32)] * self.n_agents
    ...
    def _get_info(self, agent):
        if hasattr(self.scenario, "benchmark_data"):
            return self.scenario.benchmark_data(agent) 
        else:
            return None

    # get observation for a particular agent
    def _get_obs(self, agent):
        return self.scenario.observation(agent)

    # get dones for a particular agent
    # unused right now -- agents are allowed to go beyond the viewing screen
    def _get_done(self, agent):
        if hasattr(self.scenario, "done"):
            return self.scenario.done(agent) 
        else:
            return None

    # get reward for a particular agent
    def _get_reward(self, agent):
        return self.scenario.reward(agent)
