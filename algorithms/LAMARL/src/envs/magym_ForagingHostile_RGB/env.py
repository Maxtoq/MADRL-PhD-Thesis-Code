...
class Env(gym.Env):
    ...
    def __init__(self, grid_shape=(15, 15), n_agents=4, n_gems=30, penalty=-0.0, 
                 ...
                 respawn_gems=True, random_respawn_prob=0.2):
        ...
        self._random_respawn_prob = random_respawn_prob




        # Hostile zone (TODO: tidy up this)
        self._hostile_zone_penalty = -2.0
        self._hostile_zone_freq = 10
        self._hostile_zone_landmark_color = [1, 1, 1]
        self._hostile_zone_size = (grid_shape[0] // 3, grid_shape[1] // 3)
        self._hostile_zone_step = 0
        self._hostile_zone_pos = None
        self._hostile_zone_centroid = None

        self._hostile_zone_positions = [
            (0, 0),  # NW
            (0, (grid_shape[1] - self._hostile_zone_size[1]) // 2),  # N
            (0, grid_shape[1] - self._hostile_zone_size[1]),  # NE
            ((grid_shape[0] - self._hostile_zone_size[0]) // 2, 0),  # W
            ((grid_shape[0] - self._hostile_zone_size[0]) // 2, (grid_shape[1] - self._hostile_zone_size[1]) // 2),  # Center
            ((grid_shape[0] - self._hostile_zone_size[0]) // 2, grid_shape[1] - self._hostile_zone_size[1]),  # E
            (grid_shape[0] - self._hostile_zone_size[0], 0),  # SW
            (grid_shape[0] - self._hostile_zone_size[0], (grid_shape[1] - self._hostile_zone_size[1]) // 2),  # S
            (grid_shape[0] - self._hostile_zone_size[0], grid_shape[1] - self._hostile_zone_size[1])  # SE
        ]
    ...
    def get_agent_obs(self):
        ...
        for agent_i in range(self.n_agents):
            ...
            # Mark hostile zone landmark if visible
            if self._hostile_zone_centroid:
                lr, lc = self._hostile_zone_centroid
                if abs(lr - pos[0]) <= obs_range and abs(lc - pos[1]) <= obs_range:
                    _local_r = lr - (pos[0] - obs_range)
                    _local_c = lc - (pos[1] - obs_range)
                    if 0 <= _local_r < _gem_pos.shape[0] and 0 <= _local_c < _gem_pos.shape[1]:
                        _gem_pos[_local_r, _local_c] = self._hostile_zone_landmark_color
    ...
    def step(self, agents_action):
        ...
        # Original gem respawn logic if enabled
        if self._respawn_gems:
            for gem_i in range(self.n_gems):
                if not self._gem_alive[gem_i]:
                    if np.random.random() < self._random_respawn_prob:
                        self.__spawn_gem(gem_i)
                        self._gem_alive[gem_i] = True

        # Update hostile zone
        self._hostile_zone_step += 1
        if self._hostile_zone_step >= self._hostile_zone_freq:
            self._hostile_zone_pos = self._hostile_zone_positions[self.np_random.randint(0, len(self._hostile_zone_positions))]
            self._hostile_zone_step = 0
            # Landmark at center of hostile zone
            r, c = self._hostile_zone_pos
            self._hostile_zone_centroid = [r + self._hostile_zone_size[0] // 2, c + self._hostile_zone_size[1] // 2]

        # Apply hostile zone penalties
        if self._hostile_zone_pos:
            hr, hc = self._hostile_zone_pos
            for i in range(self.n_agents):
                ar, ac = self.agent_pos[i]
                if hr <= ar < hr + self._hostile_zone_size[0] and hc <= ac < hc + self._hostile_zone_size[1]:
                    rewards[i] += self._hostile_zone_penalty

        # Only terminate when max steps are reached (gems may be 0)
        ...
        return self.get_agent_obs(), rewards, self._agent_dones, {'n_gem_alive': sum(self._gem_alive)}
    ...
    def render(self, mode='human'):
        ...
        if self._hostile_zone_pos:
            hr, hc = self._hostile_zone_pos
            for r in range(hr, hr + self._hostile_zone_size[0]):
                for c in range(hc, hc + self._hostile_zone_size[1]):
                    fill_cell(img, (r, c), cell_size=CELL_SIZE, fill='red', margin=0.1)
            if self._hostile_zone_centroid:
                draw_circle(img, self._hostile_zone_centroid, cell_size=CELL_SIZE, fill='black')
