...
class Parser():
    ...
    def _gen_perfect_message(self, agent_obs):
        ...
        pos = agent_obs[:2]
        obs_map = np.array(
            agent_obs[2:-1]).reshape((self.obs_range, self.obs_range, 3))
        ...
        for e_i in range(min(ent.shape[0], self.max_gems_in_message)):
            if np.array_equal(ent[e_i], ENT_COLORS["green"]):
                e = ["Green", "Gem"]
            elif np.array_equal(ent[e_i], ENT_COLORS["purple"]):
                e = ["Purple", "Gem"]
            elif np.array_equal(ent[e_i], ENT_COLORS["danger"]):
                e = ["Danger"]
            ...
            card = False
            if abs_pos[e_i][0] <= 0.25:
                e.append("North")
                card = True
            elif abs_pos[e_i][0] >= 0.75:
                e.append("South")
                card = True
            if abs_pos[e_i][1] <= 0.25:
                e.append("West")
                card = True
            elif abs_pos[e_i][1] >= 0.75:
                e.append("East")
                card = True
